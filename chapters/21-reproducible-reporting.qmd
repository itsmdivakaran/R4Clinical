...
```{r}
#| echo: false
# Define example params so inline `r params$...` expressions in the
# verbatim template block below can be evaluated during book rendering.
params <- list(
...
ae_filtered <- ae %>%
  filter(AESEV %in% c(params$min_aesev, "SEVERE"))

# Create summary table
ae_table <- ae_filtered %>%
  select(USUBJID, AETERM, AESEV, AEONSET = AESTDTC) %>%
  distinct() %>%
  arrange(AEONSET) %>%
  head(20)
...
```{r}
#| include: false
# Stub values so inline `r ...` expressions in the template prose below evaluate cleanly
params <- list(treatment_arm = "Treatment A", data_cutoff = Sys.Date(),
               include_serious_ae = TRUE, min_aesev = "MODERATE")
n_subjects <- 120L; pct_female <- 42.5; mean_age <- 45.3; sd_age <- 12.1
ae_timeline <- data.frame(cumulative_pct = c(10, 20, 30))
sae_count <- 8L; total_ae_events <- 150L; n_ae_subjects <- 95L
ae_incidence_pct <- 79.2
```
...
As shown in the cumulative AE incidence figure above, AE incidence accumulated over the study period,
reaching `r if(exists("ae_timeline")) tail(ae_timeline$cumulative_pct, 1) else "N/A"`% by the data cutoff.
